"use strict"; (function(window, document, jQuery, CodeMirror) {
	var Mgly = {};
	Mgly.Timer = function() {
		var self = this;
		self.start = function() {
	...
	function jqpd() {
        ...
        //var ND1 = window["location"]["href"]["substr"](0,19)
		...
		diff: function(added, removed) {
			...
			for (var i = 0; i < changes.length; ++i) {
				...
				if (change.op != 'a') {
					...
					if (change.op == 'd') lchange += ' ';
					else if (li > 0 && change.op == 'c') li += 1;
	...
	jQuery.extend(Mgly.diff.prototype, {
		...
		_lcs: function(lhs_ctx, lhs_lower, lhs_upper, rhs_ctx, rhs_lower, rhs_upper, vector_u, vector_d) {
			while ((lhs_lower < lhs_upper) && (rhs_lower < rhs_upper) && (lhs_ctx.codes[lhs_lower] == rhs_ctx.codes[rhs_lower])) {++lhs_lower; ++rhs_lower
			}
			while ((lhs_lower < lhs_upper) && (rhs_lower < rhs_upper) && (lhs_ctx.codes[lhs_upper - 1] == rhs_ctx.codes[rhs_upper - 1])) {--lhs_upper; --rhs_upper
			}
	...
	Mgly.CodeMirrorDiffView = function(el, options) {
		CodeMirror.defineExtension('centerOnCursor',
		function() {
	...
	jQuery.extend(Mgly.CodeMirrorDiffView.prototype, {
		...
		bind: function(el) {
			...
			this.editor[this.id + '-lhs'].on('change',
			function() {
				if (self.settings.autoupdate) self._changing(self.id + '-lhs', self.id + '-rhs')
			});
			this.editor[this.id + '-lhs'].on('scroll',
			function() {
				self._scrolling(self.id + '-lhs')
			});
			this.editor[this.id + '-rhs'].on('change',
			function() {
				if (self.settings.autoupdate) self._changing(self.id + '-lhs', self.id + '-rhs')
			});
			this.editor[this.id + '-rhs'].on('scroll',
			function() {
		...
		_clear: function() {
			var self = this,
			name, editor, fns, timer, i, change, l;
		...
		_markup_changes: function(editor_name1, editor_name2, changes) {
			...
			this.element.find('.merge-button').on('click',
			function(ev) {
				var side = 'rhs';
				var oside = 'lhs';
				...
				jQuery.each(info.bgClass.split(' '),
				function(i, clazz) {
		...
		_draw_diff: function(editor_name1, editor_name2, changes) {
			...
			jQuery.each(this.element.find('canvas'),
			function() {
